...
export default class PlatformManager {
      ...
      .on(
        ...
        (
          ...
          if (
            ...
            PlatformManager.topMostY - tile.displayHeight / 2
          ) {
  ...
  spawnPlatformInitial(textureKey: ITextureKey) {
    ...
    this.game.events.emit(
      GameEvents.TopmostPlatformChanged,
      topMostPlatformInfo
    );
  }
  ...
  despawnTopmostPlatform() {
    ...
    topMost.forEach((tile) => {
      if (tile.itemType) {
        const itemSprite: ITile = this._pool.spawn(
          tile.x,
          tile.y,
          tile.texture.key,
          0,
  ...
  destroyTopmostPlatform() {
    ...
    const randIdx = Algorithm.randomIntFromInterval(
      0,
      this.textureKeyArr.length - 1
    );
